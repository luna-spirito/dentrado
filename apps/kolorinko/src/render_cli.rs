...
async fn render_owned(
    ...
    // Exactly this core's owned pages, each resolved, rendered, and sent on
    // in turn — page resolution is a local snapshot walk with no cross-core
    // await phases, so a plain sequential loop keeps the core's single
    // runtime saturated.
